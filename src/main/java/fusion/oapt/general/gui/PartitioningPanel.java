    ...
    private void SuggestButtonActionPerformed(ActionEvent evt) {
    	...
    	if (NameAddressOnt.length() <= 0){
            ...
            return;
    		  ...
    		  FindOptimalClusterIntractive OP;
    		  if(con.CheckBuildModel==false)
    			  OP=new FindOptimalClusterIntractive(NameAddressOnt);
    		  else
    			   OP=new FindOptimalClusterIntractive(con.MB);
    		 ...
    		 // optimalNumCH = FindOptimalClusterIntractive.FindOptimalClusterFunc(NameAddressOnt);	  
    		  ...
    		  SelectCH CH=new SelectCH(OP.getModelBuild());
    		  CH.Run();
