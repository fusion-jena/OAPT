...
public class GraphFrame extends javax.swing.JFrame {
     ...
     */
    public GraphFrame(MainFrame mainframe, Controller con) {
        initComponents();
     ...
     */
    public void createGraph(Network network, String Sch) {
    	pauseVisualization();
        
        ModelBuild.colorIndex++;
        if (ModelBuild.colorIndex >= COLORS.length) { // We define 13 standard colors, if the number of cluster exceed more than 13, we create new randomly color
        	ran1 = new Random(); 
            ...
            color = COLORS[ModelBuild.colorIndex];
        }
        ...
        for (Vertex vertex : network.getVertices()) {
            ...
            if (Sch!= "CH"){
            	...
            	for (int a=0;a<Coordinator.KNumCH;a++){
                	if (vertex.getLabel().equals(Coordinator.CH.get(a).getLocalName())){
                		vertexColors.setColor(jungVertex, Color.BLACK, color.RED); //set the color of CHs with Red
                		chColor = true;
                	}
                }
